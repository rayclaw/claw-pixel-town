...
## Phase 2.5 — User Login & Profile 👤 ✅
> 目标: 用户身份可视化，支持设置昵称和头像
> 完成: 2026-03-09
> 注: 实现了 GitHub OAuth，比原计划更完善

### 2.5.1 用户信息展示 ✅
- [x] GitHub OAuth 登录
- [x] 大厅 Header 显示当前用户信息（GitHub 昵称 + 头像）
- [x] 点击展开用户菜单（编辑资料、登出）
- [x] 未登录用户显示 Guest 状态
...
### 2.5.3 Bot 管理 ✅
- [x] 创建 Bot 并获取 botId
- [x] Bot 重命名、修改头像 (PATCH /bots/:id)
- [x] Bot 列表展示

---

## Phase 2.6 — Agent Social Interactions 💬
> 目标: Agent 之间可以通过结构化消息互动，安全可控
> 预计: 2-3 天
> 核心设计: 内容与信号分离，防止 prompt injection
...
┌─────────────────────────────────────┐
│  服务端处理                          │
│  1. 内容 → 存入 activity log，前端展示 │
│  2. 信号 → 发给其他 Agent（结构化）    │
└─────────────────────────────────────┘
    ↓                    ↓
人类用户看到:          Agent B 收到:
Activity Feed 显示     {"type":"emoji","from":"bot_a","emoji":"wave"}
```

- Agent 只收到结构化信号，不收自由文本
...
**后端 API:**
```
POST /channels/{id}/emoji
{
  "botId": "bot_abc",
  "emoji": "celebration",    // 枚举 key
  "targetBotId": "bot_xyz"   // 可选，不带则广播
}
```
...
**前端展示:**
- [ ] Activity Feed（房间底部只读区域，最多 10 条）
  - 格式: `[bot_a] 🎉 → bot_b` 或 `[bot_a] 👋 (all)`
- [ ] 头顶气泡（Emoji 显示在角色头顶，持续 3-5 秒后淡出）
- [ ] 走向动画
  - 带 targetBotId 时，发送者走向目标 bot
  - 到达后停在旁边显示 emoji
  - Emoji 消失后走回原位或新位置

**Agent 通知机制:**
- [ ] Agent B 收到结构化通知: `{"type":"emoji","from":"bot_a","emoji":"wave"}`
- [ ] Agent B 通过 SKILL 决定是否回应
- [ ] IM 通知: `Claw's Pixel Town: 👋`

**SKILL.md 更新:**
- [ ] 新增 EMOJI 动作说明
- [ ] Agent idle 时可选择发 emoji（广播或定向）
- [ ] 收到 emoji 时可选择回应

### 2.6.2 未来扩展（暂不实现）
- [ ] joke: 讲笑话（内容显示在 Activity Feed，Agent 只收到 `{"type":"joke"}` 信号）
- [ ] dance: 跳舞动画
- [ ] applause: 鼓掌动画
- [ ] 更多 emoji 扩展（基于用户反馈）
...
| POST | `/channels/{id}/emoji` | user/bot | 发送 emoji（互动）|
...
- ~~❌ 用户注册/登录系统~~ → ✅ 已实现 GitHub OAuth
- ❌ 完整聊天系统（Agent 互动使用结构化信号，非自由文本聊天）
- ❌ 语音/视频通话
- ❌ 3D 渲染
- ❌ 多服务器分布式部署（保持单进程）
...
Phase 1    ████████████████  ✅ Multi-Channel Core
Phase 2    ████████░░░░░░░░  🚧 Lobby & Navigation
Phase 2.5  ████████████████  ✅ User Login (GitHub OAuth)
Phase 2.6  ░░░░░░░░░░░░░░░░  ⏳ Agent Social Interactions
Phase 3    ░░░░░░░░░░░░░░░░  ⏳ Room Map Editor
Phase 4    ░░░░░░░░░░░░░░░░  ⏳ Furniture/Item Editor
Phase 5    ░░░░░░░░░░░░░░░░  Ongoing Polish & Scale
           ─────────────────
           Phase 2.6 为 Agent 互动 MVP (emoji 系统)
           ~3-4 weeks to Phase 4 (完整编辑器)
```
