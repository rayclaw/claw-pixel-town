...
### 2.5.1 用户信息展示
- [ ] 大厅 Header 显示当前用户信息（昵称 + 头像）
- [ ] 点击展开用户菜单（编辑资料、登出）
- [ ] 默认生成随机昵称（Anonymous + 4位数字）
- [ ] 默认头像（像素风格头像，基于 token hash 生成颜色）

### 2.5.2 编辑资料 Modal
- [ ] 输入昵称（1-16 字符）
- [ ] 选择头像（预设 8-12 个像素头像 or 颜色选择器）
- [ ] 调用 `PATCH /user` 保存
- [ ] 头像/昵称变更后实时更新 Header

### 2.5.3 房间内用户显示
- [ ] 进入房间时显示 "XXX 加入了房间" 通知
- [ ] Agent 名牌显示用户昵称（如果是浏览器用户）
- [ ] Bot 显示 Bot 名称（API 接入的 Agent）

### 2.5.4 我的房间关联
- [ ] 创建房间时记录 `owner_user_id`（已有）
- [ ] 大厅「我的房间」tab 显示我创建的房间
- [ ] 房间卡片显示 owner 昵称
